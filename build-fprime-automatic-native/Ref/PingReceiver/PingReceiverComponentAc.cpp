...
namespace Ref {
  ...
  FwIndexType PingReceiverComponentBase ::
    ...
    )
  {
    ...
    // Serialize for IPC
    _status = msg.serializeFrom(static_cast<FwEnumStoreType>(CMD_PR_STOPPINGS));
    FW_ASSERT (
      _status == Fw::FW_SERIALIZE_OK,
      static_cast<FwAssertArgType>(_status)
    );
  ...
  void PingReceiverComponentBase ::
    log_ACTIVITY_HI_PR_PingsDisabled() const
  {
    // Get the time
    Fw::Time _logTime;
  ...
  void PingReceiverComponentBase ::
    log_DIAGNOSTIC_PR_PingReceived(U32 code) const
  {
    // Get the time
    Fw::Time _logTime;
    if (this->m_Time_OutputPort[0].isConnected()) {
...
#endif
      _status = _logBuff.serializeFrom(code);
      FW_ASSERT(
        _status == Fw::FW_SERIALIZE_OK,
        static_cast<FwAssertArgType>(_status)
      );
    ...
    doDispatch()
  {
    ...
    if (_msgType == PINGRECEIVER_COMPONENT_EXIT) {
    ...
    switch (_msgType) {
      ...
      case CMD_PR_STOPPINGS: {
        ...
        // Call handler function
        this->PR_StopPings_cmdHandler(_opCode, _cmdSeq);
