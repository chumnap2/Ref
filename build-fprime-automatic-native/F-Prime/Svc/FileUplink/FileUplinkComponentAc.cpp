...
namespace Svc {
  ...
  FwIndexType FileUplinkComponentBase ::
  ...
  {
    return static_cast<FwIndexType>(FW_NUM_ARRAY_ELEMENTS(this->m_bufferSendOut_OutputPort));
  }
    ...
    )
  {
    ...
    if (this->m_InvalidReceiveModeThrottle >= EVENTID_INVALIDRECEIVEMODE_THROTTLE) {
      ...
      (void) this->m_InvalidReceiveModeThrottle.fetch_add(1);
    }
...
#if FW_OBJECT_NAMES == 1
      const char* _formatString =
        "(%s) %s: Packet type %" PRIu32 " received in mode %" PRIu32 "";
#else
      const char* _formatString =
        "%s: Packet type %" PRIu32 " received in mode %" PRIu32 "";
#endif
    ...
    )
  {
    ...
    if (this->m_PacketOutOfBoundsThrottle >= EVENTID_PACKETOUTOFBOUNDS_THROTTLE) {
      ...
      (void) this->m_PacketOutOfBoundsThrottle.fetch_add(1);
    }
    ...
    )
  {
    ...
    if (this->m_PacketOutOfOrderThrottle >= EVENTID_PACKETOUTOFORDER_THROTTLE) {
      ...
      (void) this->m_PacketOutOfOrderThrottle.fetch_add(1);
    }
    ...
    log_WARNING_HI_PacketDuplicate(U32 packetIndex)
  {
    ...
    if (this->m_PacketDuplicateThrottle >= EVENTID_PACKETDUPLICATE_THROTTLE) {
      ...
      (void) this->m_PacketDuplicateThrottle.fetch_add(1);
    }
...
#if FW_OBJECT_NAMES == 1
      const char* _formatString =
        "(%s) %s: Invalid packet received. Wrong packet type: %" PRIu32 "";
#else
      const char* _formatString =
        "%s: Invalid packet received. Wrong packet type: %" PRIu32 "";
#endif
