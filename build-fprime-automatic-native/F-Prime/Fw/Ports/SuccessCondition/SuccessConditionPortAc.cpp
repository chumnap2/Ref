// ======================================================================
// \title  SuccessConditionPortAc.cpp
// \author Generated by fpp-to-cpp
// \brief  cpp file for SuccessCondition port
// ======================================================================

#include "Fw/Ports/SuccessCondition/SuccessConditionPortAc.hpp"
#include "Fw/Types/Assert.hpp"
#include "Fw/Types/ExternalString.hpp"

namespace Fw {

  namespace {

    // ----------------------------------------------------------------------
    // Port buffer class
    // ----------------------------------------------------------------------

    class SuccessConditionPortBuffer : public Fw::SerializeBufferBase {

      public:

        Fw::Serializable::SizeType getBuffCapacity() const {
          return InputSuccessConditionPort::SERIALIZED_SIZE;
        }

        U8* getBuffAddr() {
          return m_buff;
        }

        const U8* getBuffAddr() const {
          return m_buff;
        }

      private:

        U8 m_buff[InputSuccessConditionPort::SERIALIZED_SIZE];

    };

  }

  // ----------------------------------------------------------------------
  // Input Port Member functions
  // ----------------------------------------------------------------------

  InputSuccessConditionPort ::
    InputSuccessConditionPort() :
      Fw::InputPortBase(),
      m_func(nullptr)
  {

  }

  void InputSuccessConditionPort ::
    init()
  {
    Fw::InputPortBase::init();
  }

  void InputSuccessConditionPort ::
    addCallComp(
        Fw::PassiveComponentBase* callComp,
        CompFuncPtr funcPtr
    )
  {
    FW_ASSERT(callComp != nullptr);
    FW_ASSERT(funcPtr != nullptr);

    this->m_comp = callComp;
    this->m_func = funcPtr;
    this->m_connObj = callComp;
  }

  void InputSuccessConditionPort ::
    invoke(Fw::Success& condition)
  {
#if FW_PORT_TRACING == 1
    this->trace();
#endif

    FW_ASSERT(this->m_comp != nullptr);
    FW_ASSERT(this->m_func != nullptr);

    return this->m_func(this->m_comp, this->m_portNum, condition);
  }

#if FW_PORT_SERIALIZATION == 1

  Fw::SerializeStatus InputSuccessConditionPort ::
    invokeSerial(Fw::SerializeBufferBase& _buffer)
  {
    Fw::SerializeStatus _status;

#if FW_PORT_TRACING == 1
    this->trace();
#endif

    FW_ASSERT(this->m_comp != nullptr);
    FW_ASSERT(this->m_func != nullptr);

    Fw::Success condition;
    _status = _buffer.deserializeTo(condition);
    if (_status != Fw::FW_SERIALIZE_OK) {
      return _status;
    }

    this->m_func(this->m_comp, this->m_portNum, condition);

    return Fw::FW_SERIALIZE_OK;
  }

#endif

  // ----------------------------------------------------------------------
  // Output Port Member functions
  // ----------------------------------------------------------------------

  OutputSuccessConditionPort ::
    OutputSuccessConditionPort() :
      Fw::OutputPortBase(),
      m_port(nullptr)
  {

  }

  void OutputSuccessConditionPort ::
    init()
  {
    Fw::OutputPortBase::init();
  }

  void OutputSuccessConditionPort ::
    addCallPort(InputSuccessConditionPort* callPort)
  {
    FW_ASSERT(callPort != nullptr);

    this->m_port = callPort;
    this->m_connObj = callPort;

#if FW_PORT_SERIALIZATION == 1
    this->m_serPort = nullptr;
#endif
  }

  void OutputSuccessConditionPort ::
    invoke(Fw::Success& condition) const
  {
#if FW_PORT_TRACING == 1
    this->trace();
#endif

#if FW_PORT_SERIALIZATION
    FW_ASSERT((this->m_port != nullptr) || (this->m_serPort != nullptr));

    if (this->m_port != nullptr) {
      this->m_port->invoke(condition);
    }
    else {
      Fw::SerializeStatus _status;
      SuccessConditionPortBuffer _buffer;

      _status = _buffer.serializeFrom(condition);
      FW_ASSERT(_status == Fw::FW_SERIALIZE_OK, static_cast<FwAssertArgType>(_status));

      _status = this->m_serPort->invokeSerial(_buffer);
      FW_ASSERT(_status == Fw::FW_SERIALIZE_OK, static_cast<FwAssertArgType>(_status));
    }
#else
    FW_ASSERT(this->m_port != nullptr);
    this->m_port->invoke(condition);
#endif
  }

}
